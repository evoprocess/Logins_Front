...
export function publicHomeScreen(app, openLogin = false) {
  ...
  const open = (organization, organizationName) => { if (!app.querySelector('.login-modal')) loginScreen(app, typeof organization === 'string' ? { organization, organizationName, lockOrganization: true } : {}); };
