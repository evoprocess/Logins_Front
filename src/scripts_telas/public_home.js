    ...
    <div class="public-page">
      ...
      <main>
        ...
        <section id="localizar-organizacao" class="flow-section locator-section public-screen" data-public-screen>
          ...
          <form id="store-search-form" class="store-search" autocomplete="off"><label for="store-search">Pesquisar loja</label><div><input id="store-search" list="store-suggestions" placeholder="Digite o nome da loja, por exemplo: lojasite"><datalist id="store-suggestions">${directory.floors.flatMap(floor => floor.organizations).filter(organization => organization.status === 'active').map(organization => `<option value="${organization.name}"></option>`).join('')}</datalist><button type="submit">Ir correndo</button></div><small id="store-search-feedback">Selecione uma sugestão para correr até a fachada.</small></form>
