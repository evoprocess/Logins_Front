...
export function publicHomeScreen(app, openLogin = false) {
    ...
    <div class="public-page">
      ...
      <main>
        ...
        <section id="localizar-organizacao" class="flow-section locator-section public-screen" data-public-screen>
          ...
          <div class="game-controls"><button data-move="-1" aria-label="Andar para esquerda">←</button><button id="enter-store" disabled>Entrar</button><button data-move="1" aria-label="Andar para direita">→</button></div><p class="game-help">Teclado: <kbd>A</kbd> <kbd>D</kbd> ou setas para andar, <kbd>E</kbd> para entrar. Você também pode clicar em uma sala.</p>
        </section>
  ...
  const open = organization => { if (!app.querySelector('.login-modal')) loginScreen(app, typeof organization === 'string' ? { organization, lockOrganization: true } : {}); };
